...
HTTP/HTTPS 是一个重要的已实现模块，但不是项目的边界。协议层将持续扩展；
WebSocket 支持目前处于开发阶段，已包含协议与基础 stream 能力。
...
| [项目路线图](roadmap.md) | 已实现范围、规划中的 WebSocket 支持和项目方向 |
...
`<libgs.h>` 会包含四个顶层模块头文件。当编译时间和依赖边界比较重要时，
...
当前版本为 `0.11.1-dev`。项目尚未达到 1.0 API 稳定里程碑，因此公共接口仍可能
