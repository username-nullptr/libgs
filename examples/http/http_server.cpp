...
//		return false; // Returning false will result in abort !!!
	...
	{
		auto &request = context.request();
		spdlog::wdebug(L"Version:{} - Method:{} - Path:{}",
						request.version(),
						libgs::http::to_method_string(request.method()),
						request.path());

		for(auto &[key,value] : request.parameters())
			spdlog::wdebug(L"Parameter: {}: {}", key, value);

		for(auto &[key,value] : request.headers())
			spdlog::wdebug(L"Header: {}: {}", key, value);

		for(auto &[key,value] : request.cookies())
			spdlog::wdebug(L"Cookie: {}: {}", key, value);

		if( request.can_read_body() )
			spdlog::wdebug(L"partial_body: {}\n", co_await request.read_all());
