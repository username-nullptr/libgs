...
public:
	...
	(body_norms_t norms, AsyncOpt async_opt, AsyncProgress async_progress, Token &&token)
	{
		...
		(auto completion_handler) mutable
		{
			...
			libgs::detail::launch_awaitable(exec, co_upload_file(std::move(self), std::move(norms),
...
private:
	...
	(const_buffer body, Token &&token, std::shared_ptr<void> owner = {})
	{
		...
		{
			using handler_t = decltype(completion_handler);
			libgs::detail::launch_awaitable(exec, co_write(std::move(self), body, std::move(owner)),
				libgs::detail::co_spawn_io_handler<size_t,handler_t,decltype(exec)>(
					std::move(completion_handler), exec));
		}, completion_token);
	}
