...
(const frame_header &header, frame_codec_config config) noexcept
{
	...
	storage[result.size++] = static_cast<std::byte> (
		(header.fin ? 0x80 : 0x00) | (rsv << 4) |
		static_cast<uint8_t>(header.op)
	);
...
sys_expected<encoded_close_payload>
...
void apply_mask(const mutable_buffer &payload, const masking_key &key,
...
sys_expected<size_t> mask_copy(const mutable_buffer &destination,
	const const_buffer &source, const masking_key &key,
	uint64_t payload_offset) noexcept
{
