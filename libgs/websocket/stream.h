...
public:
	...
	basic_stream &adopt (
		connection_ptr<Exec0> connection, adopt_options_t options
	) requires
...
public:
	...
	// Waits for every data write queued before this call.
	template <typename Token = use_sync_t>
...
public:
	...
	// disabled, the latest unobserved Ping is retained for the application.
	template <typename Token = use_sync_t>
	...
	// Repeated calls join an in-progress close. After the stream is closed they
	// complete immediately and successfully with the retained close_info.
	template <typename Token = use_sync_t>
