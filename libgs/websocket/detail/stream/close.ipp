...
// Graceful Close handshake and close observers.
...
void stream_impl<Exec>::start_close_deadline() noexcept
{
	...
	try
	{
		...
		m_close_timer->expires_after(std::chrono::duration_cast<
			asio::steady_timer::duration>(m_config.close_timeout));
...
template <core_concepts::exec Exec>
void stream_impl<Exec>::finish_close(error_code error, bool clean,
	bool cancel_transport) noexcept
{
	if(m_state == connection_state::closed or
...
void stream_impl<Exec>::start_close_receive() noexcept
{
	...
	try
	{
		asio::co_spawn(self->m_exec, [self]() -> awaitable<error_code>
			...
			} }, asio::bind_executor(self->m_exec, [self](const std::exception_ptr &exception, error_code error)
					 {
...
template <core_concepts::exec Exec>
auto stream_impl<Exec>::close(close_frame frame, error_code &error) noexcept
	-> close_info_t
{
	error.clear();
	...
	for(;;)
	{
		...
		auto size = m_connection->read(mutable_buffer(
...
{
	auto completion = asio::any_completion_handler<void(error_code, close_info_t)>(
...
void stream_impl<Exec>::async_close(close_frame frame, Handler &&handler)
{
	...
	{
		auto error = m_state == connection_state::failed ? (m_error ? m_error : make_error_code(std::errc::io_error)) : make_error_code(errc::not_open);
