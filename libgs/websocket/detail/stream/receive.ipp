...
auto stream_impl<Exec>::consume_frame_data() noexcept -> sys_expected<optional<received_event>>
{
		...
		if(is_data_opcode(header.op) or header.op == opcode::continuation)
		{
			...
			if( m_config.max_message_size != 0 and
				static_cast<size_t>(header.payload_size) >
					m_config.max_message_size - std::min(m_message_body.size(), m_config.max_message_size) )
				return sys_unexpected(make_error_code(errc::message_too_big));
		}
	}
	...
	return optional<received_event>(std::move(event));
}
...
message stream_impl<Exec>::read(error_code &error) noexcept
{
	...
	for(;;)
	{
		...
		const auto size = m_connection->read (
...
void stream_impl<Exec>::async_read_message(Handler &&handler)
{
	...
	if(self->m_state != connection_state::open or self->m_read_active)
	{
		auto error =
			...
			self->m_state == connection_state::failed ? self->m_error :
	...
	try
	{
		asio::co_spawn(self->m_exec, [self]() -> awaitable<std::tuple<error_code, message>>
		{
			for(;;)
			{
				if( self->m_local_close_initiated and self->m_state == connection_state::closing )
					...
					[self, owner](auto next_handler) mutable
					{
						self->m_connection->read (
							...
							asio::any_completion_handler
								<void(error_code,size_t)>(std::move(next_handler))
						);
					},
		...
		asio::bind_executor(self->m_exec, asio::bind_cancellation_slot(waiter->cancellation.slot(),
	...
	{
		self->m_read_active = false;
		self->complete_read_waiter(exception_error(std::current_exception()));
	}
}
