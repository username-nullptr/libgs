...
public:
	...
	{
		uint64_t id = 0;
		asio::any_completion_handler<void(error_code,control_event_t)> completion;
	};
	...
	{
		uint64_t id = 0;
		asio::any_completion_handler<void(error_code,close_info_t)> completion;
	};

	enum class wire_frame_kind : uint8_t {
		data, application_control, automatic_pong, local_close, close_response,
		protocol_close,
	};
...
public:
	...
	void adopt(connection_ptr connection, adopt_options_t options,
	...
	[[nodiscard]] bool queue_has_capacity(send_kind kind,
	...
	void deliver_write_waiter(std::shared_ptr<write_waiter> waiter,
		error_code error, bool clear_slot = true) noexcept;
	...
	[[nodiscard]] size_t write_control(opcode op, const const_buffer &payload,
		error_code &error) noexcept;
	...
	void complete_control_waiter(error_code error, control_event_t event = {},
	...
	[[nodiscard]] sys_expected<std::optional<received_event>>
	...
	void complete_read_waiter(error_code error, message value = {},
		bool clear_slot = true) noexcept;
	void cancel_read_waiter(uint64_t id,
	...
	[[nodiscard]] static std::optional<close_code>
	protocol_failure_code(error_code error) noexcept;
	void begin_protocol_failure(error_code error,
		bool synchronous = false) noexcept;
...
public:
	...
	// Valid and consumed byte counts for the current transport read block.
	...
	std::optional<message_type> m_message_type;
	std::vector<std::byte> m_message_body;
	std::vector<std::byte> m_control_body;
	std::optional<control_event_t> m_control_event;
	std::shared_ptr<control_wait_operation> m_control_waiter;
	uint64_t m_next_control_waiter_id = 0;
	std::optional<std::vector<std::byte>> m_pending_auto_pong;
	std::optional<prepared_frame> m_pending_local_close;
	std::optional<std::vector<std::byte>> m_pending_close_response;
	std::optional<prepared_frame> m_pending_protocol_close;
	std::string m_subprotocol;
	std::vector<extension> m_extensions;
	std::optional<close_info_t> m_peer_close;
	std::optional<close_info_t> m_close_result;
	...
	std::optional<std::pair<uint64_t,error_code>> m_unobserved_write_error;
