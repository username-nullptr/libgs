	...
	-> sys_expected<prepared_frame>
{
	try
	{
		...
		const bool borrowed = borrow_payload and not header.mask and payload.size() != 0;
	...
	try
	{
		...
		for(const auto &buffer : buffers)
		{
			...
			if(type == message_type::text and
				not utf8.consume(std::string_view(
					reinterpret_cast<const char*>(data), buffer.size())))
				return sys_unexpected(make_error_code(protocol_errc::invalid_utf8));
		}
		if(m_config.max_message_size != 0 and
			body_size > m_config.max_message_size)
		...
		const auto frame_count = body_size == 0 or fragment_size == 0 ? 1 :
			1 + (body_size - 1) / fragment_size;
		...
		do
		{
			...
			const auto payload_size = fragment_size == 0 ? remaining :
			...
			{
				while(buffer_index < buffers.size() and
					buffer_offset == buffers[buffer_index].size())
				{
					++buffer_index;
					buffer_offset = 0;
				}
				...
				{
					auto copied = mask_copy(mutable_buffer(
						prepared.wire->data() + encoded->size + frame_offset, size),
