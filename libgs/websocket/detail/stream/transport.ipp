...
// Transport ownership and adoption.
...
template <core_concepts::exec Exec>
void stream_impl<Exec>::adopt(connection_ptr connection, adopt_options_t options,
	error_code &error) noexcept
{
	...
	if(options.stream_role != role::client and
		options.stream_role != role::server)
	{
		error = make_error_code(std::errc::invalid_argument);
		return;
	}
	...
	try
	{
		...
		m_read_buffer = std::make_shared<std::vector<std::byte>>(
			m_config.read_buffer_size);
		m_read_size = 0;
		m_read_offset = 0;
		m_pending_offset = 0;
		m_read_error.clear();
