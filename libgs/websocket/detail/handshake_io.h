...
[[nodiscard]] LIBGS_WEBSOCKET_TAPI awaitable<std::unique_ptr<handshake_io_completion<Value>>>
co_handshake_io_with_timeout(Initiator initiation, std::chrono::nanoseconds timeout)
{
...
(const Exec &exec, Initiator initiation, std::chrono::milliseconds timeout, Fallback fallback, Token &&token)
{
	...
	return asio::async_initiate<token_t,void(error_code,Value)>([exec,
		operation = std::move(initiation), effective_timeout, make_fallback = std::move(fallback)
	](auto completion_handler) mutable
	{
		...
		asio::co_spawn(exec, co_handshake_io_with_timeout<Value>(
			std::move(operation), effective_timeout), asio::bind_allocator(allocator,
